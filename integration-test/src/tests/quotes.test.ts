 ...
 * Tests: /v3/quotes/{symbol}, /v2/ticks/stocks/nbbo/{symbol}/{date}, /v2/last/nbbo/{symbol}
 */
  ...
  describe.skip('GET /v2/ticks/stocks/nbbo/{symbol}/{date} [DEPRECATED]', () => {
    describe('Required Parameters', () => {
      it.each(config.testSymbols)(
        ...
        async (symbol) => {
          const polygonResponse = await polygonClient.getHistoricQuotes(symbol, testDate, {
            limit: 10,
          });
          const simulatorResponse = await simulatorClient.getHistoricQuotes(symbol, testDate, {
            limit: 10,
          });
