...
inline nlohmann::json format_order(const Order& o) {
    ...
    result["qty"] = o.qty.value_or(0.0);
    result["filled_qty"] = o.filled_qty;
    result["filled_avg_price"] = o.filled_qty > 0 ? nlohmann::json(o.last_fill_price)
    ...
    result["limit_price"] = o.limit_price.has_value() ? nlohmann::json(*o.limit_price)
                                                      : nlohmann::json(nullptr);
    result["stop_price"] = o.stop_price.has_value() ? nlohmann::json(*o.stop_price)
                                                    : nlohmann::json(nullptr);
    result["status"] = order_status_to_string(o.status);
    result["extended_hours"] = false;
    result["legs"] = nullptr;
    result["trail_percent"] = o.trail_percent.has_value() ? nlohmann::json(*o.trail_percent)
                                                          : nlohmann::json(nullptr);
    result["trail_price"] = o.trail_price.has_value() ? nlohmann::json(*o.trail_price)
                                                      : nlohmann::json(nullptr);
    result["hwm"] = o.hwm.has_value() ? nlohmann::json(*o.hwm) : nlohmann::json(nullptr);
    return result;
}
...
inline nlohmann::json format_account(const AccountState& st, const std::string& session_id) {
    ...
    result["pending_transfer_in"] = "0";
    result["pending_transfer_out"] = "0";
